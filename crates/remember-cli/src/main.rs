...
async fn run_health(args: &[String]) -> Result<()> {
    let transport = parse_flag_value(args, "--transport").unwrap_or_else(|| "named_pipe".to_string());
    let response = invoke(
        &transport,
        "series.list",
    ...
    .await?;
    let parsed: Value = serde_json::from_str(&response).context("failed to parse health response")?;
...
async fn run_rpc(args: &[String]) -> Result<()> {
    ...
    let payload: Value =
        serde_json::from_str(&payload_raw).with_context(|| format!("invalid payload json: {payload_raw}"))?;
    let transport = parse_flag_value(args, "--transport").unwrap_or_else(|| "named_pipe".to_string());
