...
| 字段 | 值 |
|---|---|
| case_id | `P4-T4-VG-PASS` / `P4-T4-VG-FAIL` / `P4-T4-IG-PASS` / `P4-T4-IG-FAIL` |
...
## 视觉门禁
### P4-T4-VG-PASS
- case_id: `P4-T4-VG-PASS`
- skill_chain: `playwright`
- target_mode: `web_url`
- setup:
  1. 确认应用已启动并可访问 `$env:TARGET_URL`。
  2. 准备会话 ID：`P4T4-VG-PASS`。
- steps:
  1. `npx --yes --package @playwright/cli playwright-cli -s=P4T4-VG-PASS open $env:TARGET_URL --browser $env:PW_BROWSER`
  2. `npx --yes --package @playwright/cli playwright-cli -s=P4T4-VG-PASS snapshot`
  3. 根据当前子任务流程完成关键操作并截图（`screenshot`）。
  4. 记录 `置顶与摘录刷新` 对应的可见结果。
  5. `npx --yes --package @playwright/cli playwright-cli -s=P4T4-VG-PASS close`
- oracle:
  1. 关键界面元素完整显示，无错位/遮挡。
  2. `置顶与摘录刷新` 对应成功态可见。
- evidence:
  - `P4-T4-VG-PASS_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.png`
  - `P4-T4-VG-PASS_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.txt`
- failure rollback:
  1. 关闭会话并保留现有截图。
  2. 标记 `FAIL/BLOCKED`，记录卡点与复现步骤。

### P4-T4-VG-FAIL
- case_id: `P4-T4-VG-FAIL`
...
- oracle:
  ...
  2. 失败后系统仍可继续操作。
- evidence:
  - `P4-T4-VG-FAIL_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.png`
  - `P4-T4-VG-FAIL_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.txt`
- failure rollback:
  1. 关闭当前会话。
...
## 交互门禁
### P4-T4-IG-PASS
- case_id: `P4-T4-IG-PASS`
- skill_chain: `playwright`
- target_mode: `web_url`
- setup:
  1. 使用会话 ID：`P4T4-IG-PASS`。
...
- evidence:
  - `P4-T4-IG-PASS_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.mp4`
  - `P4-T4-IG-PASS_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.txt`
- failure rollback:
  1. 保存最后一步前后状态截图。
  2. 标记缺陷并附复现步骤。

### P4-T4-IG-FAIL
- case_id: `P4-T4-IG-FAIL`
...
- oracle:
  ...
  2. 系统不崩溃，合法操作可继续完成。
- evidence:
  - `P4-T4-IG-FAIL_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.mp4`
  - `P4-T4-IG-FAIL_$env:RUN_DATE_$env:ENV_ID_$env:TESTER.txt`
- failure rollback:
  1. 关闭会话并重启应用。
  2. 若复测仍失败，升级为阻断级。
